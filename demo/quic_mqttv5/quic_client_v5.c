...
extern QUIC_TLS_SECRETS g_tls_secrets;
...
conf_quic config_user = {
	.tls = {
		...
		.certfile = "/home/adam/github/quic-project/selfsigned.crt",
		.keyfile  = "/home/adam/github/quic-project/selfsigned.pem",
		.key_password = "",
		.verify_peer = false,
		.set_fail = true,
	},
...
void write_key_log_file()
{
	const char * path = getenv("SSLKEYLOGFILE");
	if (path != NULL) {
		printf("Writing keylog file to %s\n", path);
		WriteSslKeyLogFile(path, &g_tls_secrets);
	}
}
